...
const Branches = () => {
  ...
  const handleDelete = (event) => {
    ...
    fetchData(`branches/${id}`, "DELETE").then((response) => {
      if (response.status === 200) {
        closeDeletePopup();
        openSuccessPopup();
        const newBranches = branches.filter((branch) => branch._id !== selectedBranch);
        setBranches(newBranches);
      } else {
        setError(response.data);
        setErrorShowPopup(true);
  ...
  return (
    <React.Fragment>
              ...
              <tbody>
                {branches.map((branch) => (
                  <tr key={branch._id}>
                    ...
                    <td>
                      <button className="btn btn-primary buttons" onClick={() => openEditPopup(branch._id)}>
                        Edit
                      </button>
                      <button className="btn btn-danger buttons" onClick={() => openDeletePopup(branch._id)}>
                        Delete
                      </button>
                    </td>
                  </tr>
      ...
      {showDeletePopup && <DeletePopup id={id} what="branch" onDelete={handleDelete} onClose={closeDeletePopup} />}
